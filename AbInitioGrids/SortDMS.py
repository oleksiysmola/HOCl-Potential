...
columns = ["grep", "rCH", "rCO", "alpha", "mux", "muz",  "point"]
# df = pd.read_csv("CH3OH-3DEnergies.dat", delim_whitespace=True, names=columns, dtype=str)
df = pd.read_csv("HOCl-DMS.dat", delim_whitespace=True, names=columns, dtype=str)
df["point"] = df["point"].astype(float).astype(int)
df2 = pd.read_csv("HOCl-DMS-energies.dat", delim_whitespace=True, names=["point", "E"])
...
df = pd.merge(df, df2, on="point", how="inner")
df["E"]  = df["E"].astype(float)*2.194746354e05
df["E"]  = df["E"] - df["E"].min()
def computeWeightsOfPoint(row, Ethresh=15e3):
    row["weight"] = (np.tanh(-6e-4 * (row["E"] - Ethresh)) + 1.002002002)/(2.002002002*max(row["E"], 10000)*0.0001)
    return row
df = df.parallel_apply(lambda x:computeWeightsOfPoint(x), result_type="expand", axis=1)
df = df[["rCH", "rCO", "alpha", "mux", "muz", "weight", "point"]]
